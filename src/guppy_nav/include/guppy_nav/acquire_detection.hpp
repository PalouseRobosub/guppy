...
inline guppy_msgs::msg::CornerDetection convertFromString(StringView str) {
  throw BT::RuntimeError("Detection port must be connected via blackboard, not passed as a string literal in XML.");
}
...
public:
    ...
    BT::NodeStatus onTick(const std::shared_ptr<guppy_msgs::msg::CornerDetectionList>& msg) override {
        if (!msg) return BT::NodeStatus::FAILURE; // no message has arrived

        auto detections = msg->detections;

        if (detections.size() == 0) return BT::NodeStatus::FAILURE; // no detections in list
