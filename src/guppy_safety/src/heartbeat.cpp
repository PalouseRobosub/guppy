...
public:
  ...
  {
    auto empty_callback = [this](geometry_msgs::msg::Twist::UniquePtr) -> void {};
    
    rclcpp::QoS watchedQos(10);
    ...
    watchedOptions.event_callbacks.deadline_callback = 
      [this](auto &) {
        this -> panic();
      };

    pub_state = this->create_publisher<guppy_msgs::msg::State>("state", 10);
    sub_cmdvel  = this->create_subscription<geometry_msgs::msg::Twist>("cmd_vel/teleop", 
        watchedQos, 
        empty_callback,
        watchedOptions);
  };
