...
{
 public:
  CanThrottleSubscriber() : Node("can_throttle_subscriber") {
    // setup CAN socket
    if (!setup_can_socket()) {
    ...
    subscription_ = this->create_subscription<std_msgs::msg::Float32>(
        "/can/id101", 10, std::bind(&CanThrottleSubscriber::callback, this, std::placeholders::_1));  // run callback whenever a message is received on the throttle topic
  }
 ...
 private:
  void callback(const std_msgs::msg::Float32::SharedPtr msg) {
    ...
    if (write(sock_, &frame, sizeof(struct can_frame)) < 0)  // send over CAN interface
    {
      ...
      RCLCPP_INFO(this->get_logger(), "Published throttle %.3f to CAN ID 0x101", value);
    }
  }
